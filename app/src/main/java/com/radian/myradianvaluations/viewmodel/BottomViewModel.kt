...
class BottomViewModelFactory(private val context: Context) : ViewModelProvider.Factory {
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(BottomViewModel::class.java)) {
            return BottomViewModel(context) as T
        }
        throw IllegalArgumentException(context.resources.getString(R.string.unknown_viewmodel))
    }
}

class BottomViewModel(private val context: Context) : ViewModel(), ApiResponseCallBack {
    var statusresponse = MutableLiveData<StatusResponse>()
    private val apiServiceProviderGeneric = ApiServiceProviderGeneric(this)
    fun updateReadFlag(postParams: HashMap<String, Any?>) {
        apiServiceProviderGeneric.postCall(
            context,
            ReturnType.POST_UPDATE_FLAG.endPoint,
            ReturnType.POST_UPDATE_FLAG,
            postParams
        )
    }
